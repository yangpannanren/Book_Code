...
for k=1:num_ch             % loop over number of channels
   ...
   tmp_t = zeros(h_len,1);
   if nlos
       Tc = exprnd(1/Lam); % First cluster random arrival (exp distribution)
   else
   ...
   while (Tc<10*Gam)  % cluster loop    
     ...
     while (Tr<10*gam) % ray loop
        ...
        Tr = Tr + exprnd(1/Lam); % (2.10)
     end
     Tc = Tc + exprnd(1/lam); % (2.11)
   end
   ...
   % Since arrival times follow an exponential distribution, 
   % the next suspected arrival time must be later and may not need to be sorted
   % t(1:np(k),k) = tmp_t(1:np(k));
   % h(1:np(k),k) = tmp_h(1:np(k));  
   % now impose a log-normal shadowing on this realization
   fac = 10^(sdi*randn/20)/sqrt(h(1:np(k),k)'*h(1:np(k),k));% 20log(X)~N(0,sid^2)
   h(1:np(k),k) = h(1:np(k),k)*fac; % (2.15)
end