...
if decision_scheme == 1
    fprintf(['sotf decision\n' ...
        '\tSNR\t \tBER\n '])
else
    ...
    for i_frame=1:1:N_frame
        ...
        for tx_time=1:648
            ...
            if rem(tx_time-1,81)==0
                H = sq05*(randn(2,2)+1i*randn(2,2));
            end
            y = H*x;
            %%%  AWGN addition %%%
            noise = sqrt(sigma2/2)*(randn(2,1)+1i*randn(2,1));
            if noise_option==1
                y = y + noise;
            end
            %%%%%%%%%%%%% MMSE Detector %%%%%%%%%%%%%%%%%%
            ...
            if (decision_scheme == 0)
                ...
                soft_bits = soft_output2x2(X_tilde);
                Ps=1;
                SINR1=(Ps*(abs((W(1,:)*H(:,1)))^2))/...
                    (Ps*( abs((W(1,:)*H(:,2)))^2 + W(1,:)*W(1,:)'*sigma2));
                SINR2=(Ps*(abs((W(2,:)*H(:,2)))^2))/...
                    (Ps*( abs((W(2,:)*H(:,1)))^2 + W(2,:)*W(2,:)'*sigma2));
