...
for i_SNR=1:length(SNRdBs)
    ...
    for i_packet=1:N_packets
        msg_symbol=randi([0,1],N_frame*b,M);
        tx_bits=msg_symbol.';
        temp=[];
        temp1=[];
        for i=1:4
            [temp1,sym_table,P]=modulator(tx_bits(i,:),b);
            temp=[temp; temp1];
        end
        X=temp.';
        ...
        for n=1:NT
            ...
            R1n = sum(H.*X1,2)/sq_NT+sigma*(randn(N_frame,1)+1i*randn(N_frame,1));
            R2n = sum(H.*X2,2)/sq_NT+sigma*(randn(N_frame,1)+1i*randn(N_frame,1));
            R3n = sum(H.*X3,2)/sq_NT+sigma*(randn(N_frame,1)+1i*randn(N_frame,1));
            R4n = sum(H.*X4,2)/sq_NT+sigma*(randn(N_frame,1)+1i*randn(N_frame,1));
            R5n = sum(H.*X5,2)/sq_NT+sigma*(randn(N_frame,1)+1i*randn(N_frame,1));
            R6n = sum(H.*X6,2)/sq_NT+sigma*(randn(N_frame,1)+1i*randn(N_frame,1));
            R7n = sum(H.*X7,2)/sq_NT+sigma*(randn(N_frame,1)+1i*randn(N_frame,1));
            R8n = sum(H.*X8,2)/sq_NT+sigma*(randn(N_frame,1)+1i*randn(N_frame,1));
...
title([str,',BER performance of space - time block coding,','Tx:',num2str(NT),',Rx:',num2str(NR)])